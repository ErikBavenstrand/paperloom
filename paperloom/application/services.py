...
def _get_legacy_categories(categories: list[model.Category]) -> list[model.Category]:
    return [
        model.Category(
...
def _resolve_categories(uow: AbstractUnitOfWork, category_strings: list[str] | None) -> list[model.Category]:
    ...
    Args:
        uow: The unit of work to manage repository transactions.
        category_strings: A list of category strings to filter the papers by, (e.g., ["cs.AI", "math.ST"]).
    ...
    Returns:
        A list of `Category` domain objects representing the categories to be used for fetching papers.
    """
...
def fetch_and_store_historical_papers(
    ...
    *,
    ...
    Returns:
        A list of `Paper` domain objects representing the historical papers fetched from the extractor.
    """
    with uow:
        categories = _resolve_categories(uow, category_strings)
        categories += _get_legacy_categories(categories)
